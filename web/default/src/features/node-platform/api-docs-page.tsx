...
1. （可选）查询余额、从主钱包充值到市场可用余额
2. 选择脚本与版本，拉取服务商报价（offers）
3. 询价（quote）得到明细价格
4. 创建订单（order）并预留资金 —— 只有参数的哈希会上链，明文不经过控制面
5. 通过 E2EE 中继把明文参数发给执行方，等待加密结果返回
6. 提交客户端回执（receipt），控制面比对双方回执后结算
...
金额单位均为 **micros**（1 USD = 1,000,000 micros）。\`client_available\` 是可用于下单的市场余额。

### 2. 从主钱包充值到市场余额（可选）

\`POST /api/ledger/recharge\`

\`\`\`json
{ "amount_micros": 1000000 }
\`\`\`

按 1:1（USD）从主钱包 quota 划转到市场可用余额。下单前需保证 \`client_available\` 足够覆盖报价总额。
...
\`MaxCustomerMicros\` 是本次最多扣费额，需 ≤ \`client_available\`。
...
\`input_hash\` 是任务参数的哈希：对参数对象做稳定 JSON 序列化后取 SHA-256，格式 \`sha256:<hex>\`。**明文参数不放在这里**，只在第 7 步经加密中继发送。
...
若 \`state\` 为 \`REFUNDED\`，表示服务商拒单、预留资金已退回。\`RESERVED\`/\`DATA_READY\`/\`RUNNING\`/\`OFFERED\` 表示可以继续执行（第 7 步）。
